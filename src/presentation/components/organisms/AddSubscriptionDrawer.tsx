...
export const AddSubscriptionDrawer: React.FC<DrawerProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      {/* Background Overlay */}
      <div 
        className={`fixed inset-0 max-w-md mx-auto bg-black/60 backdrop-blur-sm z-40 transition-opacity duration-300 ${isOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`} 
        onClick={onClose}
      />
      
      {/* Central Success Toast */}
      {successToast && (
         <div className="fixed top-12 left-1/2 transform -translate-x-1/2 bg-accent text-background px-6 py-3 rounded-full shadow-2xl z-[60] flex items-center gap-2 font-bold text-sm max-w-md w-[80%] mx-auto justify-center animate-in fade-in zoom-in duration-300">
      ...
      <div className={`fixed bottom-0 left-1/2 transform -translate-x-1/2 w-full max-w-md bg-zinc-900 rounded-t-[2.5rem] shadow-2xl z-50 transition-transform duration-500 ease-out px-8 py-10 border-t border-white/10 ${isOpen ? 'translate-y-0' : 'translate-y-full'}`}>
